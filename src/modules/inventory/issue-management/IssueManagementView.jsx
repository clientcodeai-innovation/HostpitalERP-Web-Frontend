...
export default function IssueManagementView() {
  const {
    issues,
    issueStock,
    items,
  ...
  const handleFormItemChange = (idx, field, val) => {
    setFormData(prev => {
      ...
      // Reset batch selection if item changes
      if (field === 'itemId') {
        updatedItem.batchNo = '';
        updatedItem.qty = 1;
      }
  ...
  const handleSave = () => {
    ...
    const invalidItems = formData.itemsList.some(itm => {
      ...
      // Validate quantity against available stock in batch
      const matchedBatch = batches.find(b => b.itemId === itm.itemId && b.batchNo === itm.batchNo);
      return !matchedBatch || itm.qty > matchedBatch.qty;
    });

    if (invalidItems) {
      tempErrors.items = 'Please verify items, batch numbers, and stock quantities (Cannot issue more than available stock)';
    }
  ...
  const columns = [
    { key: 'id', label: 'Issue Slip', sortable: true, width: '13%' },
    { key: 'date', label: 'Issue Date', sortable: true, width: '15%' },
    { key: 'department', label: 'Target Department', sortable: true, width: '22%' },
    {
      key: 'items',
      label: 'Items Issued',
      sortable: false,
    ...
    { key: 'issuedBy', label: 'Authorized By', sortable: true, width: '17%' },
    {
      key: 'status',
      label: 'Status',
      sortable: true,
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-card/40 backdrop-blur-md p-4 rounded-xl border border-border">
        <SearchBar value={search} onChange={setSearch} placeholder="Search issues..." className="sm:max-w-xs" />
        <button
      ...
      <DataTable
        ...
        data={issues}
        searchQuery={search}
        searchKeys={['id', 'department', 'issuedBy']}
        actions={actions}
        emptyMessage="No departmental stock issues logged."
      />
      ...
      <DrawerForm
        ...
        title="Issue Stock to Department/Ward"
        onSubmit={handleSave}
        submitLabel="Authorize Dispatch"
        width="lg"
      >
        <FormGroup label="Target Department / Ward Area" error={errors.department} required>
          <FormSelect
          ...
          >
            ...
            <option value="OT-1 Surgery">OT-1 Surgery</option>
          </FormSelect>
        </FormGroup>

        <FormGroup label="Authorized Remarks / Purpose">
          <FormInput
            value={formData.remarks}
            onChange={(e) => setFormData(prev => ({ ...prev, remarks: e.target.value }))}
            placeholder="e.g. Replenishment request"
          />
        </FormGroup>
        ...
        <div className="space-y-3 border-t pt-4">
          ...
          <div className="space-y-3.5 max-h-60 overflow-y-auto scrollbar-thin pr-1">
            {formData.itemsList.map((itm, idx) => {
              // Get active QC Passed batches for selected item
              const itemBatches = batches.filter(b => b.itemId === itm.itemId && b.qty > 0 && b.qcStatus === 'Passed');
                    ...
                    >
                      ...
                      {itemBatches.map(b => (
                        <option key={b.id} value={b.batchNo}>
                          {b.batchNo} (In Stock: {b.qty})
                        </option>
                      ))}
                    </FormSelect>
                  </FormGroup>

                  <FormGroup label="Issue Qty" className="w-full sm:w-24">
                    <FormInput
                      type="number"
                      min="1"
                      value={itm.qty}
      ...
      {/* Details Modal */}
      <Modal isOpen={isDetailsOpen} onClose={() => setIsDetailsOpen(false)} title={`Issue Details: ${currentRecord?.id}`} size="lg">
        {currentRecord && (
          <div className="space-y-4 text-xs font-semibold text-muted-foreground font-medium">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-3 bg-muted/20 border rounded-xl">
              <div><span>Date Issued:</span><p className="text-foreground mt-0.5">{currentRecord.date}</p></div>
              <div><span>Target Department:</span><p className="text-foreground mt-0.5">{currentRecord.department}</p></div>
              <div><span>Authorized By:</span><p className="text-foreground mt-0.5">{currentRecord.issuedBy}</p></div>
            ...
            {currentRecord.remarks && (
              <div className="p-3 bg-muted/10 border border-dashed rounded-lg">
                ...
                <p className="text-foreground mt-1">{currentRecord.remarks}</p>
              </div>
            )}

            <div className="space-y-2 border-t pt-3">
              <h4 className="text-xs font-bold text-foreground">Dispatched Lot details</h4>
                    ...
                    {currentRecord.items.map((itm, i) => {
                      ...
                      return (
                        <tr key={i} className="hover:bg-muted/10 text-foreground font-medium">
                          ...
                          <td className="p-3 font-mono font-semibold">{itm.batchNo}</td>
                          <td className="p-3 text-right font-mono font-bold text-primary">{itm.qty}</td>
                        </tr>
                      );
                    })}
                  </tbody>
